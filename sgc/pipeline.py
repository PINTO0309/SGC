...
from .model import SGC, ModelConfig

LOGGER = logging.getLogger("sgc")

LABEL_MAP = {0: "no_sunglasses", 1: "sunglasses"}
                ...
                results.append(
                    {
                        ...
                        "prob_sunglasses": float(probs[idx].detach().cpu().item()),
                    }
                )
    return results
...
def train_pipeline(config: TrainConfig, verbose: bool = False) -> Dict[str, Any]:
    ...
    train_dataset = SGCDataset(train_samples, transform=train_transform)
    val_dataset = SGCDataset(splits["val"], transform=eval_transform) if splits["val"] else None
    test_dataset = SGCDataset(splits["test"], transform=eval_transform) if splits["test"] else None
    ...
    model = SGC(model_config).to(device)
    base_metadata = {
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        epoch_path = config.output_dir / f"sgc_epoch_{epoch:04d}.pt"
        torch.save(epoch_payload, epoch_path)
        _prune_checkpoints(config.output_dir, "sgc_epoch_", 10)
        ...
        if score_value > best_f1:
            ...
            best_checkpoint_path = config.output_dir / f"sgc_best_epoch{epoch:04d}_f1_{score_value:.4f}.pt"
            best_state["checkpoint_path"] = str(best_checkpoint_path)
            torch.save(best_checkpoint, best_checkpoint_path)
            _prune_checkpoints(config.output_dir, "sgc_best_", 10)
...
def export_to_onnx(
    ...
    torch.onnx.export(
        ...
        output_path,
        input_names=["images"],
        output_names=["prob_sunglasses"],
        dynamic_axes={"images": {0: "batch"}, "prob_sunglasses": {0: "batch"}},
        opset_version=opset,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SGC training and inference pipeline")
    subparsers = parser.add_subparsers(dest="command", required=True)

    train_parser = subparsers.add_parser("train", help="Train the SGC binary classifier from parquet data.")
