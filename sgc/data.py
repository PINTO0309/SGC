...
class Sample:
    """Lightweight container describing a single sunglasses-classification image."""
...
POSITIVE_LABELS = {"sunglasses", "with_sunglasses"}
NEGATIVE_LABELS = {"no_sunglasses", "without_sunglasses"}
...
def collect_samples(data_root: Path, logger: Optional[logging.Logger] = None) -> List[Sample]:
    ...
    for idx, row in df.iterrows():
        ...
        label = _to_label(row)
        label_name = str(row.get("label", "sunglasses" if label == 1 else "no_sunglasses"))
...
def split_samples(
    ...
    for split, subset in assignments.items():
        ...
        logger.info(
            "Split '%s': %d samples from %d sources (%.1f%% labelled 'sunglasses').",
            split,
            total,
...
class SGCDataset(Dataset):
    """Torch dataset that reads sunglasses-classification images from disk or embedded bytes."""
